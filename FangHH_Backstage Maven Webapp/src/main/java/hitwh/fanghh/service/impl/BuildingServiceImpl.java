...
public class BuildingServiceImpl implements BuildingService
{
	...
	public int getBuildingPageNum(int pageMaxNum)
	{
		...
		return buildingNum % pageMaxNum == 0 ? buildingNum / pageMaxNum : buildingNum / pageMaxNum + 1;
	}
	...
	{
		Surroundfacility surroundfacility = new Surroundfacility(buildingSurroundBean);
		Surroundfacility s = SurroundfacilityDao.selectByBuildingId(buildingSurroundBean.getBuildingId());
		if (s != null)
		{
			return false;
		}
		...
		if (result == 1)
		{
			...
			if (DictionaryString.APPLICATION_TOBEPERFERT.equals(building.getBuildingApplicationStatus()))
	...
	{
		Surroundfacility surroundfacility = new Surroundfacility(buildingSurroundBean);
		Integer result = SurroundfacilityDao.updateByPrimaryKeySelective(surroundfacility);
		if (result == 1)
		{
			Building building = buildingDao.selectByPrimaryKey(buildingSurroundBean.getBuildingId());
			if (DictionaryString.APPLICATION_TOBEPERFERT.equals(building.getBuildingApplicationStatus()))
	...
	{
		Surroundfacility surroundfacility = SurroundfacilityDao.selectByBuildingId(buildingId);
		if (surroundfacility == null)
			return false;
		Integer result = SurroundfacilityDao.deleteByPrimaryKey(surroundfacility.getSfId());
		if (result == 1)
			return true;
		return false;
	}
