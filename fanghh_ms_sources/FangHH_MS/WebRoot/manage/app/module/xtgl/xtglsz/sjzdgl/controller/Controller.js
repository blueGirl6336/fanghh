Ext.define('HRMS.module.xtgl.xtglsz.sjzdgl.controller.Controller',
		{
			...
			views : [ 'HRMS.module.xtgl.xtglsz.sjzdgl.view.DictionaryGroupList',
					'HRMS.module.xtgl.xtglsz.sjzdgl.view.DictionaryItemList' ],
			...
			getDictionary : function(grid, record) {
				...
				store.load();
				var button = this.getDictionarygrouplist().down(
						'button[action=delete]');
				button.setDisabled(false);
			},

			prepareDelete : function(grid, record) {
				var button = this.getDictionaryitemlist().down(
						'button[action=delete]');
				button.setDisabled(false);
			},

			addDictionaryItem : function() {
				var smRecord = this.getDictionarygrouplist()
						.getSelectionModel().getSelection()[0];
				if (smRecord == null) {
					Ext.MessageBox.alert('警告：', '请先选择左侧的字典组再添加！');
					return;
				}
